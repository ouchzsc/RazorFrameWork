...
namespace Aio
{
    public sealed class Octets : IComparable<Octets>
    {
        ...
        public  Octets WrapBytes(byte[] bytes, int length)
        {
            ByteArray = bytes;
            Count = length;
            return this;
        ...
        {
           if(Count > 0)
                EraseAndCompact(Count, DefaultSize);

            return this;
        }

        public Octets ResetWithSize(int reserveSize)
        {
            if (Count > 0)
                EraseAndCompact(Count, reserveSize);
